...
class BacktrackingHeuristicSearch():
    ...
    def performBacktrackSearch(self, rootNode, node):
        ...
        if node.state.checkGoalState():
            ...
            variable = node.state.selectHeuristicUnassignedVariable()
            self.showImage(node.state)
            
            for value in node.state.getHeuristicOrderDomainValues(variable):
...
bts = BacktrackingHeuristicSearch(True)
bts.search()